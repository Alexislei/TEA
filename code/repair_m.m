...
function off=repair_m(off,ch)

    %处理超出变量域的数值
    off(off>24)=24;off(off<1)=1;
    %处理empty space
    [m,n]=size(ch);
...
% function off=repair(off,c1)
%     missItem=setdiff(c1,intersect(off,c1));
%     while ~isempty(missItem)
%         mask=zeros(size(c1,1),size(c1,2));
%         for i=1:size(missItem,1)
%             mask=mask+(c1==missItem(i,:));
%         end
%         
%         off=(1-mask).*off+mask.*c1;
%         
%         missItem=setdiff(c1,intersect(off,c1));
%     end
% end
